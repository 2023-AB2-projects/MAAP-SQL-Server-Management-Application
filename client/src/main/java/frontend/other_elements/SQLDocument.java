...
public class SQLDocument extends DefaultStyledDocument {
    ...
    private final MutableAttributeSet normal, keyword, operator, type, comment, quote;
    private final HashMap<String, Object> keywords, operators, types;
    ...
    public SQLDocument() {
        ...
        Color green = new Color(91, 250, 91);
        StyleConstants.setForeground(comment, green);
        StyleConstants.setItalic(comment, true);

        // Keywords
        keyword = new SimpleAttributeSet();
        Color blue = new Color(79, 162, 255);
        StyleConstants.setForeground(keyword, blue);

        // Operators
        operator = new SimpleAttributeSet();
        StyleConstants.setForeground(operator, new Color(252, 85, 165));

        // Attributes
        type = new SimpleAttributeSet();
        StyleConstants.setForeground(type, new Color(11, 180, 204));
        ...
        for (final String keyword : this.RESERVED_KEYWORDS) {
            keywords.put(keyword, dummyObject);
        }

        operators = new HashMap<>();
        for (final String operator : this.OPERATORS) {
            operators.put(operator, dummyObject);
        }

        types = new HashMap<>();
        for (final String type : this.ATTRIBUTE_TYPES) {
            types.put(type, dummyObject);
        }
    }
    ...
    protected boolean isOperator(String token) {
        Object o = operators.get(token);
        return o != null;
    }

    protected boolean isType(String token) {
        Object o = types.get(token);
        return o != null;
    }
