...
public class CatalogManager {
    ...
    public static String getIndexName(String databaseName, String tableName, String field) throws NoIndexException {
        List<IndexFileModel> indexes = getUniqueIndexes(databaseName, tableName);
        indexes.addAll(getNonUniqueIndexes(databaseName, tableName));

        for (var index : indexes) {
            if (index.getIndexFields().size() == 1 && index.getIndexFields().contains(field)) {
